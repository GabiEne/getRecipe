...
use Application\Form\Client\LoginForm;
use Application\Form\Client\LoginFilter;
...
class AuthController extends AbstractActionController
{
	public function indexAction(){
		...
		$view= new ViewModel();
		$view->setTemplate('application/client/auth/index');
		return $view;
...
//-					if ($data['rememberme']) $authService->getStorage()->session->getManager()->rememberMe($time); // no way to get the session
		...
		$view->setTemplate('application/client/auth/login');
		return $view;
    }
    ...
    public function logoutAction()
    {
    	...
    	// return $this->redirect()->toRoute('home');
    	return $this->redirect()->toRoute('client/index3', array('controller' => 'auth', 'action' => 'login'));
